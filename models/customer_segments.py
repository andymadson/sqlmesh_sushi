# File: models/customer_segments.py
...
@model(
    ...
    owner="analytics",
    grains=["customer_id"],
    # Make lineage explicit so the DAG always shows this model downstream of CLV.
    depends_on=["sushimoderate.customer_lifetime_value"],
    columns={
        "customer_id": "int",
...
def execute(context: ExecutionContext, **kwargs: Any) -> Iterator[pd.DataFrame]:
    ...
    clv_table = context.resolve_table("sushimoderate.customer_lifetime_value")
    ...
    def silhouette_scores(x: np.ndarray, labels: np.ndarray) -> np.ndarray:
        # Pairwise Euclidean distances (OK for small N typical of the sushi example)
        n = x.shape[0]
